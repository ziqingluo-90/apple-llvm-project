...
static cl::opt<bool> DeprecatedDriverCommand("deprecated-driver-command",
...
static int scanDeps(ArrayRef<const char *> Args, std::string WorkingDirectory,
  ...
  auto HandleCommand = [&](CXString ContextHash, CXStringSet *ModuleDeps, CXStringSet *FileDeps, CXStringSet *Args) {
    llvm::outs() << "  command " << CommandIndex++ << ":\n";
    llvm::outs() << "    context-hash: " << clang_getCString(ContextHash)
                 << "\n"
                 << "    module-deps:\n";
    for (const auto &ModuleName : llvm::makeArrayRef(ModuleDeps->Strings,
                                                     ModuleDeps->Count))
      llvm::outs() << "      " << clang_getCString(ModuleName) << "\n";
    llvm::outs() << "    file-deps:\n";
    for (const auto &FileName :
  ...
  if (options::DeprecatedDriverCommand) {
    CXFileDependencies *Result = clang_experimental_DependencyScannerWorker_getFileDependencies_v3(
    ...
    if (Result) {
      ...
      HandleCommand(Result->ContextHash, Result->ModuleDeps, Result->FileDeps, Result->BuildArguments);
      clang_experimental_FileDependencies_dispose(Result);
    }
  } else {
    CXFileDependencies_v4 *Result = clang_experimental_DependencyScannerWorker_getFileDependencies_v4(
        Worker, Args.size(), Args.data(),
        ModuleName ? ModuleName->c_str() : nullptr, WorkingDirectory.c_str(),
        CB.Context, CB.Callback, LookupOutputCB.Context, LookupOutputCB.Callback,
        /*Options=*/0, &Error);
    if (Result) {
      ...
      for (size_t I = 0; I < Result->NumCommands; ++I)
        HandleCommand(Result->Commands[I].ContextHash, Result->Commands[I].ModuleDeps, Result->Commands[I].FileDeps, Result->Commands[I].BuildArguments);
      clang_experimental_FileDependencies_dispose_v4(Result);
    }
  }
