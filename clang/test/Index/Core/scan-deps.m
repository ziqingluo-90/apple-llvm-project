...
// CC1-NEXT:       build-args: -cc1 {{.*}} -fmodule-file={{.*}}ModA_{{.*}}.pcm
// DRIVER-NEXT:    build-args: -c {{.*}} -save-temps=obj {{.*}} -fno-implicit-modules -fno-implicit-module-maps -fmodule-file={{.*}}ModA_{{.*}}.pcm
// SAVETEMPS-NEXT: build-args: -cc1 {{.*}} -E -fmodule-file={{.*}}ModA_{{.*}}.pcm
...
// SAVETEMPS-NEXT:   build-args: -cc1 {{.*}} -emit-llvm-bc -fmodule-file={{.*}}ModA_{{.*}}.pcm
