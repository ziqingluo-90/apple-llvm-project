...
// RUN: %deps-to-rsp %t/deps.json --tu-index 2 --tu-cmd-index 0 > %t/tu-cpp.rsp
// RUN: %deps-to-rsp %t/deps.json --tu-index 2 --tu-cmd-index 1 > %t/tu-emit-ir.rsp
// RUN: %deps-to-rsp %t/deps.json --tu-index 2 --tu-cmd-index 2 > %t/tu-emit-asm.rsp
// RUN: %deps-to-rsp %t/deps.json --tu-index 2 --tu-cmd-index 3 > %t/tu-cc1as.rsp
// RUN: %clang @%t/Mod.rsp
...
// CHECK-NEXT:           "-cc1"
// CHECK:                "-o"
// CHECK-NEXT:           "{{.*}}tu_no_integrated_as{{.*}}.s"
// CHECK:                "-S"
// CHECK:                "{{.*}}tu_no_integrated_as.c"
// CHECK:              ]
...
// CHECK-NEXT:         "command-line": [
// CHECK:                "-o"
// CHECK-NEXT:           "{{.*}}tu_no_integrated_as.o"
// CHECK:                "{{.*}}tu_no_integrated_as{{.*}}.s"
// CHECK:              ]
// CHECK-NEXT:         "executable": "/usr/bin/as"
// CHECK-NEXT:       }
// CHECK-NEXT:     ]
// CHECK:          "input-file": "[[PREFIX]]{{.}}tu_no_integrated_as.c"
// CHECK:        }
// CHECK-NEXT:   {
// CHECK:          "clang-module-deps": []
// CHECK:          "commands": [
...
[
  {
    "directory": "DIR"
    "command": "clang -c DIR/tu_no_integrated_as.c -no-integrated-as -o DIR/tu_no_integrated_as.o"
    "file": "DIR/tu_no_integrated_as.c"
  },
  {
    "directory": "DIR"
    "command": "clang -c DIR/tu_no_integrated_cpp.c -no-integrated-cpp -o DIR/tu_no_integrated_cpp.o"
    "file": "DIR/tu_no_integrated_cpp.c"
  },
  {
    "directory": "DIR"
    "command": "clang -c DIR/tu_save_temps_module.c -save-temps=obj -o DIR/tu_save_temps_module.o -fmodules -fimplicit-modules -fimplicit-module-maps"
...
void tu_save_temps(void) { foo(); }

//--- tu_no_integrated_cpp.c
#include "plain_header.h"
void tu_no_integrated_cpp(void) { foo(); }

//--- tu_no_integrated_as.c
#include "plain_header.h"
void tu_no_integrated_as(void) { foo(); }
